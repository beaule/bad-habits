...
module.exports = function(BadHabit) { 
   ...
   BadHabit.analysis= function(product,startDate,callback){
    ...
    BadHabit.notify= function(product,callback){
     ...
     //load Bad habits from API sandbox
     BadHabit.import= function(callback){
        ...
        request({
            ...
            if (!error && response.statusCode == 200) {
                ...
                var map = {
                    ...
                    operate: [
                    {   run: function(val) {    var arr=val.split("###"); console.log(arr);if(arr.length>0) return arr[0]; else return "";}, on: "category"  },
                ...
                var transformedResult = dataTransform.transform();
                var fs = require('fs');
                fs.writeFile("test", JSON.stringify(transformedResult), function(err) {
                    if(err) {
        ...
        callback(null,"List of transaction imported");
    }; 
    ...
    BadHabit.remoteMethod(
        'import', {
            ...
            description:'Load data from api sandbox',
            returns: [
                {arg: 'List of transactions', type: 'string'},
          ]
        }
